...
class ApiKeyAuthenticate
{
    ...
    {
        $accessKey = $request->header('Access-Key'); // Assuming the access key is included in the request headers

        if (!$accessKey) {
            return response()->json(['status' => false, 'message' => 'Invalid access key'], 403);
        }
